...
If you go to the browser and refresh the page you will see a list of links. The first three items are links that can go to different locations on the web.  The last item is not a link to demonstrate the different ways we can use our HTML elements. Now we need a navigation bar. Can you guess what tag we are going to use? You is right, we gonna use a `<nav>` tag. Since we want to navigate to different part os our site we are going to use `a` tags as well. 
