...
Save your changes in the file, and restart the server (go to terminal, press `CTRL-C` to close the session, and then run `node bin/www`. If you go to the browser and refresh the page you will see a list of links. The first three items are links that can go to different locations on the web.  The last item is not a link to demonstrate the different ways we can use our HTML elements. Now we need a navigation bar. Can you guess what tag we are going to use? You is right, we gonna use a `<nav>` tag. Since we want to navigate to different part os our site we are going to use `a` tags as well. 
...
Ok before we move forward. I got to confess I'm a bit lazy (as are most programmers). I noticed that every time anytime we make a change to our file and want to see the changes we have to refresh the browser. I find this to be mad work. Since we're software developers can we find a better way to do this? The answer is yes, and you guessed it we're going to use a tool that someone else created since they encountered the same problem. We are going to use a tool called `nodemon`. This cool little invention  
