...
Download the virtualization software [Virtualbox](https://virtualbox.org/wiki/Downloads). Virtualbox allows us to run different operating system that are different from the ones we are currently using. If this doesn't make sense now you'll understand as soon as the installation is finished.
